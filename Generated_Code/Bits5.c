...
} Bits5_TDeviceData;                   /* Device data structure type */

typedef Bits5_TDeviceData *Bits5_TDeviceDataPtr ; /* Pointer to the device data structure. */

static const uint32_t Bits5_PIN_MASK_MAP[2U] = {
   0x40U, 0x80U
};                                     /* Map of masks for each pin */

/* {Default RTOS Adapter} Static object used for simulation of dynamic driver memory allocation */
static Bits5_TDeviceData DeviceDataPrv__DEFAULT_RTOS_ALLOC;
...
LDD_TDeviceData* Bits5_Init(LDD_TUserData *UserDataPtr)
{
  ...
  Bits5_TDeviceDataPtr DeviceDataPrv;
  ...
  /* GPIOB_PDDR: PDD|=0xC0 */
  GPIOB_PDDR |= GPIO_PDDR_PDD(0xC0);
  /* Set initialization value */
  /* GPIOB_PDOR: PDO&=~0xC0 */
  GPIOB_PDOR &= (uint32_t)~(uint32_t)(GPIO_PDOR_PDO(0xC0));
  ...
  PE_LDD_RegisterDeviceStructure(PE_LDD_COMPONENT_Bits5_ID,DeviceDataPrv);
...
void Bits5_SetDir(LDD_TDeviceData *DeviceDataPtr, bool Dir)
{
  ...
  if (Dir) {
    /* Output */
    GPIO_PDD_SetPortOutputDirectionMask(Bits5_MODULE_BASE_ADDRESS, Bits5_PORT_MASK);
  } else {
    /* Input */
    GPIO_PDD_SetPortInputDirectionMask(Bits5_MODULE_BASE_ADDRESS, Bits5_PORT_MASK);
  }
}
...
uint32_t Bits5_GetVal(LDD_TDeviceData *DeviceDataPtr)
{
  ...
  (void)DeviceDataPtr;                 /* Parameter is not used, suppress unused argument warning */
  if ((GPIO_PDD_GetPortDirection(Bits5_MODULE_BASE_ADDRESS) & Bits5_PORT_MASK) != 0U) {
    /* Port is configured as output */
    PortData = GPIO_PDD_GetPortDataOutput(Bits5_MODULE_BASE_ADDRESS) & Bits5_PORT_MASK;
  } else {
    /* Port is configured as input */
    PortData = GPIO_PDD_GetPortDataInput(Bits5_MODULE_BASE_ADDRESS) & Bits5_PORT_MASK;
  }
  return PortData >> Bits5_PIN_ALLOC_0_INDEX; /* Return port data shifted with the offset of the first allocated pin*/
}
...
void Bits5_PutVal(LDD_TDeviceData *DeviceDataPtr, uint32_t Val)
{
  ...
  RawVal = (Val & Bits5_PORT_VALID_VALUE_MASK) << Bits5_PIN_ALLOC_0_INDEX; /* Mask and shift output value */
  /* Set port data by toggling the different bits only */
  GPIO_PDD_TogglePortDataOutputMask(Bits5_MODULE_BASE_ADDRESS,
      (GPIO_PDD_GetPortDataOutput(Bits5_MODULE_BASE_ADDRESS) ^ RawVal) & Bits5_PORT_MASK);
}
...
LDD_TError Bits5_GetBit(LDD_TDeviceData *DeviceDataPtr, uint8_t Bit, bool *BitVal)
{
  ...
  Mask = Bits5_PIN_MASK_MAP[Bit];
  if ((GPIO_PDD_GetPortDirection(Bits5_MODULE_BASE_ADDRESS) & Mask) != 0U) {
    /* Port is configured as output */
    PortVal = GPIO_PDD_GetPortDataOutput(Bits5_MODULE_BASE_ADDRESS);
  } else {
    /* Port is configured as input */
    PortVal = GPIO_PDD_GetPortDataInput(Bits5_MODULE_BASE_ADDRESS);
  }
  if ((PortVal & Mask) != 0U) {
    *BitVal = (bool)TRUE;
  } else {
...
LDD_TError Bits5_PutBit(LDD_TDeviceData *DeviceDataPtr, uint8_t Bit, bool Val)
{
  ...
  Mask = Bits5_PIN_MASK_MAP[Bit];
  if (Val) {
    GPIO_PDD_SetPortDataOutputMask(Bits5_MODULE_BASE_ADDRESS, Mask);
  } else { /* !Val */
    GPIO_PDD_ClearPortDataOutputMask(Bits5_MODULE_BASE_ADDRESS, Mask);
  } /* !Val */
  return ERR_OK;
}
