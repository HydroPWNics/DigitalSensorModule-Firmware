...
**          Pins                                           : 1
**            Pin0                                         : 
**              Pin                                        : ADC0_SE13/PTB13/TPM1_CH1/RTC_CLKOUT
**              Pin signal                                 : 
...
} Bits3_TDeviceData;                   /* Device data structure type */

typedef Bits3_TDeviceData *Bits3_TDeviceDataPtr ; /* Pointer to the device data structure. */

static const uint32_t Bits3_PIN_MASK_MAP[1U] = {
   0x2000U
};                                     /* Map of masks for each pin */

/* {Default RTOS Adapter} Static object used for simulation of dynamic driver memory allocation */
static Bits3_TDeviceData DeviceDataPrv__DEFAULT_RTOS_ALLOC;
...
LDD_TDeviceData* Bits3_Init(LDD_TUserData *UserDataPtr)
{
  ...
  Bits3_TDeviceDataPtr DeviceDataPrv;
  ...
  PE_LDD_RegisterDeviceStructure(PE_LDD_COMPONENT_Bits3_ID,DeviceDataPrv);
...
void Bits3_SetDir(LDD_TDeviceData *DeviceDataPtr, bool Dir)
{
  ...
  if (Dir) {
    /* Output */
    GPIO_PDD_SetPortOutputDirectionMask(Bits3_MODULE_BASE_ADDRESS, Bits3_PORT_MASK);
  } else {
    /* Input */
    GPIO_PDD_SetPortInputDirectionMask(Bits3_MODULE_BASE_ADDRESS, Bits3_PORT_MASK);
  }
}
...
uint32_t Bits3_GetVal(LDD_TDeviceData *DeviceDataPtr)
{
  ...
  (void)DeviceDataPtr;                 /* Parameter is not used, suppress unused argument warning */
  if ((GPIO_PDD_GetPortDirection(Bits3_MODULE_BASE_ADDRESS) & Bits3_PORT_MASK) != 0U) {
    /* Port is configured as output */
    PortData = GPIO_PDD_GetPortDataOutput(Bits3_MODULE_BASE_ADDRESS) & Bits3_PORT_MASK;
  } else {
    /* Port is configured as input */
    PortData = GPIO_PDD_GetPortDataInput(Bits3_MODULE_BASE_ADDRESS) & Bits3_PORT_MASK;
  }
  return PortData >> Bits3_PIN_ALLOC_0_INDEX; /* Return port data shifted with the offset of the first allocated pin*/
}
...
void Bits3_PutVal(LDD_TDeviceData *DeviceDataPtr, uint32_t Val)
{
  ...
  RawVal = (Val & Bits3_PORT_VALID_VALUE_MASK) << Bits3_PIN_ALLOC_0_INDEX; /* Mask and shift output value */
  /* Set port data by toggling the different bits only */
  GPIO_PDD_TogglePortDataOutputMask(Bits3_MODULE_BASE_ADDRESS,
      (GPIO_PDD_GetPortDataOutput(Bits3_MODULE_BASE_ADDRESS) ^ RawVal) & Bits3_PORT_MASK);
}
...
LDD_TError Bits3_GetBit(LDD_TDeviceData *DeviceDataPtr, uint8_t Bit, bool *BitVal)
{
  ...
  Mask = Bits3_PIN_MASK_MAP[Bit];
  if ((GPIO_PDD_GetPortDirection(Bits3_MODULE_BASE_ADDRESS) & Mask) != 0U) {
    /* Port is configured as output */
    PortVal = GPIO_PDD_GetPortDataOutput(Bits3_MODULE_BASE_ADDRESS);
  } else {
    /* Port is configured as input */
    PortVal = GPIO_PDD_GetPortDataInput(Bits3_MODULE_BASE_ADDRESS);
  }
  if ((PortVal & Mask) != 0U) {
    *BitVal = (bool)TRUE;
  } else {
...
LDD_TError Bits3_PutBit(LDD_TDeviceData *DeviceDataPtr, uint8_t Bit, bool Val)
{
  ...
  Mask = Bits3_PIN_MASK_MAP[Bit];
  if (Val) {
    GPIO_PDD_SetPortDataOutputMask(Bits3_MODULE_BASE_ADDRESS, Mask);
  } else { /* !Val */
    GPIO_PDD_ClearPortDataOutputMask(Bits3_MODULE_BASE_ADDRESS, Mask);
  } /* !Val */
  return ERR_OK;
}
